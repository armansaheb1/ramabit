...
# def cpanel(request):
#     if(len(Verify.objects.filter(user = request.user))<1):
#         verify = Verify(user = request.user)
#         verify.save()
...
#     wall = wallet.objects.filter(user = request.user)
#     bi = bid.objects.filter(user = request.user)
#     for bii in bi :
        
#         bids.append([bii.plan.currency.pic , bii.deposit ,bii.date_field, bii.plan.period ,bii.plan.percent , bii.id])
    
#     for cur in currencies.objects.all():
#         if(len(wallet.objects.filter(user = request.user , currency= cur))>0):
#             wal = wallet.objects.get(user = request.user , currency= cur)
...
#             act = 'واریز'
#             actc= 'green'
#         trans.append([currencies.objects.get(id = transa.currency.id).pic, transa.amount,transa.date,act,actc])
...
#     for eplan in profitlist.objects.filter(user = request.user):
...
#             cur = currencies.objects.get(id = eplan.currency.id).pic 
#         pro.append([User.objects.get(id = eplan.user.id).name + User.objects.get(id = eplan.user.id).lastname , plant ,eplan.amount,eplan.date,cur])
#     inv = ''
    
#     if(len(User.objects.filter(ref = request.user.inv))):
...
#     acts=''
#     for item in Subjects.objects.filter(user = request.user):
#         if item.act == 0 :
#             acts = 'در حال بررسی'
#         if item.act == 1 :
#             acts = 'پاسخ داده شده'
...
#     unread = 0 
#     for item in Subjects.objects.filter(user = request.user , read = False):
#         unread = unread + 1
#     arz = 0 
#     for item in wallet.objects.filter(user = request.user ):
#         arz = arz + 1
#     fplan = 0 
#     for item in bid.objects.filter(user = request.user):
#         fplan = fplan + 1
#     plans = []
#     for item1 in currencies.objects.all():
#         plans.append([Plans.objects.filter(currency = item1) , item1.name , item1.pic])
...
#             act = 'واریز'
#             actc= 'green'
#         trans.append([currencies.objects.get(id = transa.currency.id).pic, transa.amount,transa.date,act,actc,User.objects.get(id = transa.user.id).name +' '+ User.objects.get(id = transa.user.id).lastname])
...
#         mellis.append([melli.user.id , melli.mellic , melli.melliimg , User.objects.get(id = melli.user.id).name , User.objects.get(id = melli.user.id).lastname , melli.id])
        

#     for bank in Adminverifybank.objects.filter(action = False):
#         banks.append([bank.user.id , bank.bankc , bank.bankimg , User.objects.get(id = bank.user.id).name , User.objects.get(id = bank.user.id).lastname , bank.id])

#     for req in Addamountreq.objects.all() :
#         cur = currencies.objects.get(id = req.currency.id).pic
#         username = User.objects.get(id = req.user.id).name
#         userlastname = User.objects.get(id = req.user.id).lastname
#         userid = User.objects.get(id = req.user.id).id
#         addamountreq.append([username , userlastname , cur , req.amount , req.link , userid , req.currency.id , req.id])
...
#         userlastname = User.objects.get(id = req.user.id).lastname
#         userid = User.objects.get(id = req.user.id).id
#         askamountreq.append([username , userlastname , cur , req.amount , req.link , userid , req.currency.id , req.id])


#     for eplan in Plans.objects.all():
#         eplans.append([eplan.title ,eplan.currency.name ,eplan.percent,eplan.period , eplan.id ])
...
#         if item.act == 3 :
#             acts = 'بسته شده'
#         subject.append([item.title , item.date , item.lastdate , acts,item.id , User.objects.get(id = item.user.id).username,item.aread])
#     unread = 0 
#     for item in Subjects.objects.filter(user = request.user , aread = False):
#         unread = unread + 1
...
def buy(request):
    ...
    option = request.POST['option']
    cur = Plans.objects.get(id=planid)
    if(len(wallet.objects.filter(user = request.user , currency = cur))>0):
        havings = wallet.objects.get(user = request.user , currency = cur)
        ...
        wal = wallet(user = request.user , currency = cur , amount = 0)
        wal.save()
        return render(request , "auctions/message.html" ,{
            "m": " دارایی کیف پول مربوط به این پلن کافی نیست لطفا کیف پول خود را شارژ کنید",
            'set' : settings.objects.get(id=1)
...
def verifymelli(request):
    ...
    if request.method == "POST":
        ...
        if(Adminverifymelli.objects.filter(user = request.user).exists()):
            veri= Adminverifymelli.objects.get(user = request.user)
            veri.melliimg = img
            veri.mellic = code
            veri.save()
        else:
            adminv = Adminverifymelli.objects.create(user = request.user , melliimg = img , mellic = code)
        user = Verify.objects.get(user = request.user)
        user.melliv = 1
        user.save()
        return render(request, "auctions/cpanelmsg.html" , {
...
def addamountreq(request):
    ...
    add = Addamountreq(link=link , amount = amount ,user = User.objects.get(id = userid) , currency = currencies.objects.get(id = curid))
    add.save()
    return render(request, "auctions/cpanelmsg.html" , {
            "m": "با موفقیت انجام شد",
            'set' : settings.objects.get(id=1)
...
def askamountreq(request):
    ...
    userid = request.POST['userid']
    curid = request.POST['curid']
    add = Askamountreq(link=link , amount = amount ,userid = userid , currency = currencies.objects.get(id = curid))
    add.save()
...
#             'set' : settings.objects.get(id=1)
#         })
#     if(wallet.objects.filter(user = userid , currency = currencies.objects.get(id = curid))):
#         wal = wallet.objects.get(user = userid , currency = currencies.objects.get(id = curid))
#         wal.amount = float(wal.amount) + float(amount)
#         wal.save()
#         req = Addamountreq.objects.get(id = idm)
#         req.delete()
#         trans = Transactions(userid = userid , currency = currencies.objects.get(id = curid) ,amount = amount , act = 1)
#         trans.save()
#     else:
#         wal = wallet(user = userid , currency = currencies.objects.get(id = curid) , amount = amount)
#         wal.save()
#         req = Addamountreq.objects.get(id = idm)
#         req.delete()
#         trans = Transactions(userid = userid , currency = currencies.objects.get(id = curid) ,amount = amount , act = 1)
...
#         if(wallet.objects.filter(user = firstinv.id , currency = currencies.objects.get(id = curid))):
#             wal2 = wallet.objects.get(user = firstinv.id , currency = currencies.objects.get(id = curid))
#             wal2.amount = float(wal2.amount) + float(amount)* 0.02
#             wal2.save()
#             trans = profitlist(userid = firstinv.id ,invid = userid, currency = currencies.objects.get(id = curid) ,amount = float(amount)* 0.02)
#             trans.save()
#         else:
#             wal2 = wallet(user = firstinv.id , currency = currencies.objects.get(id = curid) , amount = float(amount)*  0.02)
#             wal2.save()
#             trans = profitlist(userid = firstinv.id ,invid = userid, currency = currencies.objects.get(id = curid) ,amount = float(amount)*   0.02)
#             trans.save()     
#         if(firstinv.inv):
#             secondinv = User.objects.get(ref = firstinv.inv)
#             if(wallet.objects.filter(user = secondinv.id , currency = currencies.objects.get(id = curid))):
#                 wal3 = wallet.objects.get(user = secondinv.id , currency = currencies.objects.get(id = curid))
#                 wal3.amount = float(wal3.amount) + float(amount)* 0.01
#                 wal3.save()
#                 trans = profitlist(userid = secondinv.id ,invid = userid, currency = currencies.objects.get(id = curid) ,amount = float(amount)* 0.01 )
#                 trans.save()
#             else:
#                 wal3 = wallet(user = secondinv.id , currency = currencies.objects.get(id = curid) , amount = float(amount)* 0.01)
#                 wal3.save()
#                 trans = profitlist(userid = secondinv.id ,invid = userid, currency = currencies.objects.get(id = curid) ,amount = float(amount)* 0.01 )
#                 trans.save()
#             if(secondinv.inv):
#                 thirdinv = User.objects.get(ref = secondinv.inv)
#                 if(wallet.objects.filter(user = thirdinv.id , currency = currencies.objects.get(id = curid))):
#                     wal4 = wallet.objects.get(user = thirdinv.id , currency = currencies.objects.get(id = curid))
#                     wal4.amount = float(wal4.amount) + float(amount)* 0.005
#                     wal4.save()
#                     trans = profitlist(userid = thirdinv.id ,invid = userid, currency = currencies.objects.get(id = curid) ,amount = float(amount)* 0.005 )
#                     trans.save()
#                 else:
#                     wal4 = wallet(user = thirdinv.id , currency = currencies.objects.get(id = curid) , amount = float(amount)* 0.005)
#                     wal4.save()
#                     trans = profitlist(userid = thirdinv.id ,invid = userid, currency = currencies.objects.get(id = curid) ,amount = float(amount)* 0.005 )
#                     trans.save()
#                     if(thirdinv.inv):
#                         fourth = User.objects.get(ref = thirdinv.inv)
#                         if(wallet.objects.filter(user = fourth.id , currency = currencies.objects.get(id = curid))):
#                             wal5 = wallet.objects.get(user = fourth.id , currency = currencies.objects.get(id = curid))
#                             wal5.amount = float(wal5.amount) + float(amount)* 0.0025
#                             wal5.save()
#                             trans = profitlist(userid = fourth.id ,invid = userid, currency = currencies.objects.get(id = curid) ,amount = float(amount)* 0.0025 )
#                             trans.save()
#                         else:
#                             wal5 = wallet(user = fourth.id , currency = currencies.objects.get(id = curid) , amount = float(amount)* 0.0025)
#                             wal5.save()
#                             trans = profitlist(userid = fourth.id ,invid = userid, currency = currencies.objects.get(id = curid) ,amount = float(amount)* 0.0025 )
#                             trans.save()
#     return render(request, "auctions/adminmsg.html" , {
...
#     req = Askamountreq.objects.get(id = idm)
#     req.delete()
#     trans = Transactions(userid = userid , currency = currencies.objects.get(id = curid) ,amount = amount , act = 2)
#     trans.save()
#     return render(request, "auctions/adminmsg.html" , {
#             "m": "با موفقیت انجام شد",
#             'set' : settings.objects.get(id=1)
...
def closeplan(request):
    ...
    if plans.period == 'half-year':
        ...
        wall = wallet.objects.get(user = bids.user.id , curid = plans.cur)
...
#     for cur in currencies.objects.all():
#         if(len(wallet.objects.filter(user = id , currency= cur))>0):
#             wal = wallet.objects.get(user = id , currency= cur)
...
def addticket(request):
    if(request.method == 'POST'):
        title = request.POST['title']
        des = request.POST['des']
        subject = Subjects(user = request.user , title = title)
        subject.save()
        if 'pic' in request.FILES:
            pic = request.FILES['pic']
...
def addjob(request):
    if(request.method == 'POST'):
        ...
        job = Job(user = request.user ,username = request.user.username , title = title , text = des ,job = jobs)
        job.save()
        return render(request, "auctions/cpanelmsg.html" , {
            "m": "تیکت شما با موفقیت ثبت شد",
            'set' : settings.objects.get(id=1)
